...
### construct a designer_specific dataframe from ONLY Asmodee
...
df_designer = pd.DataFrame(columns=['designer', 'mechanic', 'category', 'family','publisher','Asmodee'])
a = 0
for designer in unique_designer:
    ...
    for index, x in enumerate(df['designer']):
        if x == designer:
            ...
            list_publisher = list_publisher + df.iloc[index]['publisher']
    ...
    df_designer.loc[a, 'publisher'] = list_publisher
    a = a + 1
...
#### FILTER OUT NON-ASMODEE ROWS
### go through publisher column, if it contains Asmodee, yields 1, otherwise 0 to the Asmodee column
for index, publisher in enumerate(df_designer['publisher']):
    if ('Asmodee' not in publisher):
        df_designer.drop(index, axis=0, inplace=True)
df_designer.reset_index(inplace=True)
...
### Save to excel file
df_designer.to_excel("Designer Specific_Asmodee.xlsx")
